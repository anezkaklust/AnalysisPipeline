...
#          /opt/jobsub_lite/bin/jobsub_submit --group minerva --lines +SingularityImage="/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest" --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --role=Analysis --expected-lifetime 96h --memory 4000MB -f /pnfs/minerva/persistent/users/anezkak/1D_modelComparison//myareatar_tag_default_1703436159.tar.gz file:///minerva/app/users/anezkak/MAT_GitHub/eventLoop_NONE_wrapper_tag_default_1703436159.sh
...
# -f files for input
    ...
    ${JSB_TMP}/ifdh.sh cp -D /pnfs/minerva/persistent/users/anezkak/1D_modelComparison//myareatar_tag_default_1703436159.tar.gz ${CONDOR_DIR_INPUT}
    chmod u+x ${CONDOR_DIR_INPUT}/myareatar_tag_default_1703436159.tar.gz
...
export JOBSUB_EXE_SCRIPT=$(ls eventLoop_NONE_wrapper_tag_default_1703436159.sh 2>/dev/null)
if [ "$JOBSUB_EXE_SCRIPT" = "" ]; then
     export JOBSUB_EXE_SCRIPT=$(find . -name eventLoop_NONE_wrapper_tag_default_1703436159.sh -print | head -1)
fi
...
${JSB_TMP}/ifdh.sh log "$JOBSUBJOBID anezkak:eventLoop_NONE_wrapper_tag_default_1703436159.sh COMPLETED with return code $JOB_RET_STATUS"
